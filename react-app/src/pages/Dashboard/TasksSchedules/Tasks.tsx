...
const TasksPage: React.FC = () => {
  ...
  return (
  <Box p={3} id="tasks-container" data-testid="tasks-container">
    {/* Assignee Filter */}
    <Box
      mb={3}
      width={250}
      id="assignee-filter-box"
      data-testid="assignee-filter-box"
    >
      <FormControl fullWidth data-testid="assignee-filter-control">
        <InputLabel
          id="assignee-filter-label"
          data-testid="assignee-filter-label"
        >
          Filter by Assignee
        </InputLabel>
        ...
        >
          ...
          {users.map((u) => (
            <MenuItem
              key={u.id}
              value={u.id}
              id={`assignee-filter-${u.id}`}
              data-testid={`assignee-filter-${u.id}`}
            >
              {u.displayName}
            </MenuItem>
          ))}
      ...
      {statuses.map((status) => (
        ...
        >
          <Typography
            ...
            data-testid={`status-title-${status}`}
          >
            {status}
          </Typography>
          ...
          {tasksByStatus(status).length === 0 && (
            <Typography
              ...
              id={`status-empty-${status}`}
              data-testid={`status-empty-${status}`}
            >
              No tasks
            </Typography>
          )}
          ...
          {tasksByStatus(status).map((task) => (
            ...
            >
              <CardContent data-testid={`task-card-content-${task.taskId}`}>
                <Typography
                  variant="subtitle1"
                  fontWeight={600}
                  id={`task-title-${task.taskId}`}
                  data-testid={`task-title-${task.taskId}`}
                >
                  {task.title}
                </Typography>
                ...
                <Typography
                  ...
                  data-testid={`task-description-${task.taskId}`}
                >
                  {task.description}
                </Typography>
                ...
                <Typography
                  ...
                  data-testid={`task-due-${task.taskId}`}
                >
                ...
                <Typography
                  ...
                  data-testid={`task-notes-${task.taskId}`}
                >
